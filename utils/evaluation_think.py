...
@torch.no_grad()        
def evaluation_test_think(out_dir, val_loader, model, eval_wrapper, draw = False, savenpy=False) : 
    ...
    for batch in tqdm(val_loader):
        ...
        for i in range(1): # this loop is for multimodality, due to the computation cost, we don't calculate multimodality here
            ...
            for k in range(bs):
                try:
                    index_motion, think_content = model.generate_motion_think(caption[k])
                    pred_pose = model.net.forward_decoder(index_motion[0])
                    if batch_idx == 1 and k == 0:
                        print('pred_pose', pred_pose)
                        print("think_content", think_content)
                        print("index_motion", index_motion)
                except:
                ...
                if i == 0 and (draw or savenpy):
                    ...
                    if savenpy:
                        ...
                        mp4_path = os.path.join(out_dir, name[k]+'_pred.mp4')
                        plot_3d_motion(mp4_path, t2m_kinematic_chain, pred_xyz.squeeze().detach().cpu().numpy(), title=caption[k], fps=20, radius=4)
                ...
                if draw or savenpy:
                    ...
                    if savenpy:
                        for j in range(bs):
                            ...
                            mp4_path = os.path.join(out_dir, name[j]+'_gt.mp4')
                            plot_3d_motion(mp4_path, t2m_kinematic_chain, pred_xyz[j][:m_length[j]].squeeze().detach().cpu().numpy(), title=caption[j], fps=20, radius=4)
